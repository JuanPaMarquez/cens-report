...
interface BarPlotDoubleProps {
  data?: { label: string; value1: number; value2: number; value3: number }[];
  labels?: { value1: string; value2: string; value3: string };
...
export default function BarPlotDouble({
  ...
  labels = { value1: "Mineral", value2: "Vegetal", value3: "En blanco" },
  colors = ["#1daade", "#45DC20", "#E6CF03"], // Colores por defecto
}: BarPlotDoubleProps) {
  const containerRef = useRef<HTMLDivElement | null>(null);
  const svgRef = useRef<SVGSVGElement | null>(null);
  const [dimensions, setDimensions] = useState({ width: 0, height: 0 });
  const [startYear, setStartYear] = useState(data[0].label);
  const [endYear, setEndYear] = useState(data[data.length - 1].label);
  const height = 300;

  const filteredData = data.filter(
    (d) => d.label >= startYear && d.label <= endYear
  );
  ...
  useEffect(() => {
    if (!filteredData || filteredData.length === 0 || dimensions.width === 0) return;

    const { width } = dimensions;
    const margin = { top: 80, right: 10, bottom: 40, left: 40 };
    ...
    const y = d3
      .scaleLinear()
      .domain([0, d3.max(filteredData, (d) => Math.max(d.value1, d.value2)) || 0])
      .nice()
      .range([innerHeight, 0]);

    const colorScale = d3.scaleOrdinal<string>().domain(["value1", "value2", "value3"]).range(colors);
    ...
    // Dibujar barras
    g.selectAll<SVGGElement, { label: string; value1: number; value2: number; value3: number }>(".group")
      .data(filteredData)
      .join("g")
      .attr("class", "group")
    ...
    const legend = svg
      .append("g")
      .attr("transform", `translate(${margin.left-30}, ${margin.top - 75})`);

    const legendData = [
      { key: "value1", color: colors[0], label: labels.value1 },
      { key: "value2", color: colors[1], label: labels.value2 },
      { key: "value3", color: colors[2], label: labels.value3 },
    ];
  ...
  }, [filteredData, dimensions, colors, labels]);

  return (
    <div ref={containerRef} className="overflow-y-auto w-full h-full">
      <div className="flex gap-4 mt-2 mb-4 flex-col md:flex-row">
        <label className="flex md:flex-col justify-between gap-2 pl-1" >
